...
def get_ai_provider(
    ...
    # 2. Check environment variables - auto-detect based on available API keys
    if not provider_type:
        env_provider = os.getenv("AI_PROVIDER", "").lower()
        if not env_provider:
            # Auto-detect: prefer Gemini if key exists, then OpenAI, then Anthropic
